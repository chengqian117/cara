...
@RestController
@RequestMapping("/cara")
@Api(value = "cara在线服务",tags = "cara在线服务")
@Slf4j
...
//        id= (id==null||id.equals(0))?1:id;
        ...
        try{
            ...
            stream.write(KeyouUtils.numberToKey(id).getBytes());
            ...
            stream.write("01".getBytes());
            stream.write("01".getBytes());
            stream.write(KeyouUtils.lengthToString(var5.length).getBytes());
            stream.write(var5);
            ...
            message = stream.toByteArray();
            response = KeyouUtils.call(socket, message);
            byte[] var6 = KeyouUtils.dealResponse(response, true, headString).getData();
