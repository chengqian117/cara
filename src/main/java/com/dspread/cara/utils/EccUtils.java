...
public class EccUtils {
     ...
     * @return
     */
    public static KeyStore generateKeyStore(String alias,String password,X500Name subject,X500Name issuer,long validity){
        validity = validity==0?1096L:validity;
        //生成秘钥
        ...
        try{
            ...
            PublicKey ada = keyfactory.generatePublic(x509EncodedKeySpec);
            System.out.println(Arrays.toString(ada.getEncoded()));
...
//            final long validity = 1096;
            //设置cer内容
            X509CertImpl cer = createCer(subject, validity, publicKey,issuer);
            cer.sign(privateKey, "SHA256withECDSA");
    ...
    public static boolean generateAndExportRoot(String alias,String password,X500Name subject,String extPath){
        extPath = initPath(extPath);
        String file=extPath + File.separator + alias + ".p12";
        File rootKeyStore = new File(file);
//        if(rootKeyStore.exists()){
//            return  false;
//        }
        KeyStore keyStore = generateKeyStore(alias, password, subject,subject,365*100);
        if(keyStore==null){
            return false;
        }
    ...
    public static boolean exportCer(String alias,String extPath,String parentAlias,String password){
        ...
        try{
            extPath = initPath(extPath);
            in = new FileInputStream(extPath+File.separator+parentAlias+".p12");
            ...
            String psB64Certificate = Base64.getEncoder().encodeToString(certificate.getEncoded());
            psB64Certificate= EccUtils.splitString(psB64Certificate,64);
            psB64Certificate="-----BEGIN CERTIFICATE-----\n"+psB64Certificate+"\n-----END CERTIFICATE-----\n";
            fop.write(psB64Certificate.getBytes());
            fop.close();
            in.close();
        }catch(Exception exception){
            exception.printStackTrace();
...
//            String random = RandomStringUtils.random(24, "0123456789abcdef");
            String random = RandomStringUtils.random(13, "0123456789");
            BigInteger add =(new BigInteger(random,10));
            cerInfo.set(X509CertInfo.SERIAL_NUMBER, new CertificateSerialNumber(add));
            AlgorithmId algorithmID = AlgorithmId.get("SHA256withECDSA");
...
//            log.error(CREATE_KEYSTORE_ERROR);
    ...
    public static PublicKey paseKey(byte[] pub){
        try{
            byte[] bs=new byte[pub.length+1];
            System.arraycopy(pub,0,bs,1,pub.length);
            bs[0]=4;
            DERBitString bitString = new DERBitString(bs);

            ASN1EncodableVector in = new ASN1EncodableVector();
            final String ecPublicKey  = "1.2.840.10045.2.1";
            final String secp256k1  = "1.2.840.10045.3.1.7";
            in.add(new ASN1ObjectIdentifier(ecPublicKey));
            in.add(new ASN1ObjectIdentifier(secp256k1));
            DERSequence inSequence = new DERSequence(in);

            ASN1EncodableVector out = new ASN1EncodableVector();
            out.add(inSequence);
            out.add(bitString);
            DERSequence outSequence = new DERSequence(out);

            byte[] encoded = outSequence.getEncoded();
            log.debug(Base64.getEncoder().encodeToString(encoded));
            KeyFactory keyFactory = KeyFactory.getInstance("EC");
            ...
            exception.printStackTrace();
            throw new RuntimeException("ECC公钥解析失败");
        }
